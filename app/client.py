...
class Client:
    ...
    def create_market_sell_order(
        ...
        """
        try:
            sell_order = self.binance_client.order_market_sell(
                symbol=order.symbol.symbol,
    ...
    def execute_market_buy_strategy(
        self,
        order: Order,
    ) -> OrderInProgress:
        """
    ...
    def execute_market_sell_strategy(
        self,
        order: Order,
    ) -> OrderInProgress:
        """
        ...
        if isinstance(order, LimitOrder):
            if not (sell_order_in_progress := self.create_limit_buy_order(order)):
                sys.exit("Limit buy order has not been created")

        elif isinstance(order, MarketOrder):
            if not (sell_order_in_progress := self.create_market_sell_order(order)):
                sys.exit("Market sell order has not been created")
        else:
            sys.exit("Order type not supported yet.")
